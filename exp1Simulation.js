...
var AND_w1 = 1;
var OR_w1=0;
var AND_w2 = 1;var OR_w2=0; var NOT_w1;
var AND_calcOP;var OR_calcOP;var NOT_calcOP;
var gate_id=1;
...
$(document).ready(function () {
    ...
    */
    ...
    OR_w1 = $("#NOT_Gate_w1_slider").slider("value");
...
function simulateNOTGate(iterationNo, inputX, w1, threshold, interval, timer) {
    ...
    */
    ...
    //$(".NOT-YVal").text(inputY[iterationNo]);
    $("#NOT-inputX-oplay_neuron1").addClass(inputX[iterationNo] == 1 ? "animatedLineGreen" : "animatedLinePurple");
    //$("#NOT-inputY-oplay_neuron1").addClass(inputY[iterationNo] == 1 ? "animatedLineGreen" : "animatedLinePurple");
...
function startSimulation(interval) {
    ...
    if(gate_id==1){
        ...
        simulateNOTGate(iterationNo, inputXofNOT.join(","), parseFloat(NOT_w1), parseFloat(OR_threshold), interval);
    }
}
...
function verifyNOTOutputs(){
    var correcONOTS=[1,0];
    if(correcONOTS.length==NOT_calcOP.length){
        for(var i=0;i<NOT_calcOP.length;i++){
            if(correcONOTS[i]!=NOT_calcOP[i]){
                alert("Incorrect Output values obtained from Neural Network!!! Try some different weights and threshold.");
                return;
            }
        }
...
function plotNOTGraph()
{
    ...
    var constPointSize=5;
    var OP1 = board.create('point',[0,1], {size:constPointSize,face:'^',fixed:true});
	var OP2 = board.create('point',[1,0], {size:constPointSize,face:'x',fixed:true});
    decisionBoundary1 = board.create('line', [NOT_threshold*-1, Number(NOT_w1)], {fixed:true});
